...
class Jamf_helper
{
    ...
    public function pull_jamf_data(&$Jamf_model)
    {
        ...
        $Jamf_model->last_pull_timestamp = time();
        ...
        $json = json_decode((string)$jamf_computer_result);
        ...
        foreach($json->computer->configuration_profiles as $profile){
            ...
            if (intval($profile->id) > 0){
                ...
                if (strpos((string)$jamf_profile_result, "Not Found") !== false) {
                    continue;
                }
                ...
                $profile_json = json_decode((string)$jamf_profile_result);
        ...
        $xml = simplexml_load_string((string)$jamf_patching_xml_result);
        ...
        if ($jamf_computerhistory_result) {
            ...
            $json = json_decode((string)$jamf_computerhistory_result);
